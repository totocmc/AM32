...
uint16_t servo_high_threshold =
    1900; // anything above this point considered 2000 (max)
...
_Static_assert(
    sizeof(FIRMWARE_NAME) <= 13,
    "Firmware name too long"); // max 12 character firmware name plus NULL
...
uint16_t VOLTAGE_DIVIDER =
    TARGET_VOLTAGE_DIVIDER; // 100k upper and 10k lower resistor in divider
uint16_t
...
uint16_t duty_cycle_maximum =
    2000; // restricted by temperature or low rpm throttle protect
uint16_t low_rpm_level =
    20; // thousand erpm used to set range for throttle resrictions
...
int32_t doPidCalculations(struct fastPID *pidnow, int actual, int target) {
  ...
  pidnow->pid_output =
      pidnow->error * pidnow->Kp + pidnow->integral + pidnow->derivative;
...
void loadEEpromSettings() {
  ...
  if (eepromBuffer.pwm_frequency < 49 && eepromBuffer.pwm_frequency > 7) {
    if (eepromBuffer.pwm_frequency < 49 && eepromBuffer.pwm_frequency > 23) {
      TIMER1_MAX_ARR = map(eepromBuffer.pwm_frequency, 24, 48, TIM1_AUTORELOAD,
                           TIM1_AUTORELOAD / 2);
    }
    if (eepromBuffer.pwm_frequency < 24 && eepromBuffer.pwm_frequency > 11) {
      TIMER1_MAX_ARR = map(eepromBuffer.pwm_frequency, 12, 24,
                           TIM1_AUTORELOAD * 2, TIM1_AUTORELOAD);
    }
    if (eepromBuffer.pwm_frequency < 12 && eepromBuffer.pwm_frequency > 7) {
      TIMER1_MAX_ARR = map(eepromBuffer.pwm_frequency, 7, 16,
                           TIM1_AUTORELOAD * 3, TIM1_AUTORELOAD / 2 * 3);
    }
  ...
  if (eepromBuffer.startup_power < 151 && eepromBuffer.startup_power > 49) {
    if (!eepromBuffer
             .comp_pwm) { // higher startup power for non-complementary pwm
...
#endif
  setVolume(2);
  if (eepromBuffer.eeprom_version >
      0) { // these commands weren't introduced until eeprom version 1.
#ifdef CUSTOM_RAMP
...
#endif
    servo_low_threshold = (eepromBuffer.servo.low_threshold * 2) +
                          750; // anything below this point considered 0
    ...
    if (eepromBuffer.sine_mode_changeover_thottle_level < 5 ||
        eepromBuffer.sine_mode_changeover_thottle_level >
            25) { // sine mode changeover 5-25 percent throttle
      eepromBuffer.sine_mode_changeover_thottle_level = 5;
    }
    if (eepromBuffer.drag_brake_strength == 0 ||
        eepromBuffer.drag_brake_strength > 10) { // drag brake 1-10
      eepromBuffer.drag_brake_strength = 10;
    }

    if (eepromBuffer.driving_brake_strength == 0 ||
        eepromBuffer.driving_brake_strength > 9) { // motor brake 1-9
      eepromBuffer.driving_brake_strength = 10;
    }

    if (eepromBuffer.driving_brake_strength < 10) {
      dead_time_override =
          DEAD_TIME + (150 - (eepromBuffer.driving_brake_strength * 10));
    ...
    if (eepromBuffer.limits.temperature < 70 ||
        eepromBuffer.limits.temperature > 140) {
    ...
    if (eepromBuffer.sine_mode_power == 0 ||
...
void getBemfState() {
  ...
  if (step == 2 ||
      step == 5) { //        in phase two or 5 read from phase A Pf1
...
void commutate() {
  ...
  bemfcounter = 0;
  zcfound = 0;
  commutation_intervals[step - 1] =
      commutation_interval; // just used to calulate average
...
void PeriodElapsedCallback() {
  ...
  commutation_interval =
      ((commutation_interval) + ((lastzctime + thiszctime) >> 1)) >> 1;
  if (!eepromBuffer.auto_advance) {
    advance = (commutation_interval >> 3) *
              temp_advance; // 60 divde 8 7.5 degree increments
  } else {
    advance = (commutation_interval * auto_advance_level) >>
              6; // 60 divde 64 0.9375 degree increments
  }
...
void interruptRoutine() {
  if (average_interval > 125) {
    if ((INTERVAL_TIMER_COUNT < 125) && (duty_cycle < 600) &&
        (zero_crosses < 500)) { // should be impossible, desync?exit anyway
      return;
    }
...
void setInput() {
  ...
  if (eepromBuffer.bi_direction) {
    if (dshot == 0) {
      if (eepromBuffer.rc_car_reverse) {
        ...
        if (newinput >= (1000 - (servo_dead_band << 1)) &&
            newinput <= (1000 + (servo_dead_band << 1))) {
          adjusted_input = 0;
          if (prop_brake_active) {
            prop_brake_active = 0;
            return_to_center = 1;
...
#ifndef BRUSHED_MODE
  if ((bemf_timeout_happened > bemf_timeout) &&
    ...
    if (eepromBuffer.use_sine_start) {
      ...
      if (adjusted_input > 30 &&
          adjusted_input <
              (eepromBuffer.sine_mode_changeover_thottle_level * 20)) {
        input = map(adjusted_input, 30,
                    (eepromBuffer.sine_mode_changeover_thottle_level * 20), 47,
                    160);
      }
      if (adjusted_input >=
          (eepromBuffer.sine_mode_changeover_thottle_level * 20)) {
        input = map(adjusted_input,
                    (eepromBuffer.sine_mode_changeover_thottle_level * 20),
                    2047, 160, 2047);
      }
    } else {
        ...
        if (drive_by_rpm) {
          ...
          if (adjusted_input < 47) { // dead band ?
            ...
            input = (uint16_t)(input_override /
                               10000); // speed control pid override
            if (input > 2047) {
              input = 2047;
            }
            if (input < 48) {
          ...
          input =
              (uint16_t)(input_override / 10000); // speed control pid override
          if (input > 2047) {
            input = 2047;
          }
          if (input < 48) {
  ...
  if (!stepper_sine && armed) {
    if (input >= 47 + (80 * eepromBuffer.use_sine_start)) {
      ...
      if (eepromBuffer.use_sine_start) {
        duty_cycle_setpoint =
            map(input, 137, 2047, minimum_duty_cycle + 40, 2000);
      } else {
          ...
          if (eepromBuffer.brake_on_stop) {
            if (!eepromBuffer.use_sine_start) {
              ...
              prop_brake_duty_cycle =
                  (1980) + eepromBuffer.drag_brake_strength * 2;
              proportionalBrake();
              prop_brake_active = 1;
#else
    ...
    if (!prop_brake_active) {
      if (input >= 47 &&
          (zero_crosses < (uint32_t)(30 >> eepromBuffer.stall_protection))) {
      ...
      if (stall_protection_adjust > 0 && input > 47) {
        ...
        duty_cycle_setpoint =
            duty_cycle_setpoint + (uint16_t)(stall_protection_adjust / 10000);
      }
    }
  }
#endif
...
void tenKhzRoutine() { // 20khz as of 2.00 to be renamed
  ...
  if (eepromBuffer.telemetry_on_interval) {
    telem_ms_count++;
    if (telem_ms_count >
        ((telemetry_interval_ms - 1 + eepromBuffer.telemetry_on_interval) *
         20)) {
      // telemetry_on_interval = 1 is a boolean, but it can also be 2 or more to
      // indicate an identifier by making the interval just slightly different
      // with an unique identifier, we can guarantee that many ESCs can
      // communicate on just one signal there will be some collisions but not as
      // many as if two ESCs always tried to talk at once.
      send_telemetry = 1;
      telem_ms_count = 0;
    }
  }
  ...
  if (!stepper_sine) {
    ...
    if (one_khz_loop_counter > PID_LOOP_DIVIDER) { // 1khz PID loop
      ...
      if (eepromBuffer.stall_protection &&
          running) { // this boosts throttle as the rpm gets lower, for crawlers
                     // and rc cars only, do not use for multirotors.
        stall_protection_adjust += (doPidCalculations(
            &stallPid, commutation_interval, stall_protect_target_interval));
    ...
    if (maximum_throttle_change_ramp) {
      ...
      uint16_t voltage_based_max_change =
          map(battery_voltage, 800, 2200, 10, 1);
    ...
    if ((armed && running) && input > 47) {
      ...
      if (prop_brake_active) {
        adjusted_duty_cycle =
            TIMER1_MAX_ARR - ((prop_brake_duty_cycle * tim1_arr) / 2000) + 1;
      } else {
...
#ifdef GIMBAL_MODE
  setPWMCompare1(((2 * pwmSin[phase_A_position]) + gate_drive_offset) *
                 TIMER1_MAX_ARR / 2000);
  setPWMCompare2(((2 * pwmSin[phase_B_position]) + gate_drive_offset) *
                 TIMER1_MAX_ARR / 2000);
  setPWMCompare3(((2 * pwmSin[phase_C_position]) + gate_drive_offset) *
                 TIMER1_MAX_ARR / 2000);
#else
  setPWMCompare1(
      (((2 * pwmSin[phase_A_position] / SINE_DIVIDER) + gate_drive_offset) *
       TIMER1_MAX_ARR / 2000) *
      eepromBuffer.sine_mode_power / 10);
  setPWMCompare2(
      (((2 * pwmSin[phase_B_position] / SINE_DIVIDER) + gate_drive_offset) *
       TIMER1_MAX_ARR / 2000) *
      eepromBuffer.sine_mode_power / 10);
  setPWMCompare3(
      (((2 * pwmSin[phase_C_position] / SINE_DIVIDER) + gate_drive_offset) *
       TIMER1_MAX_ARR / 2000) *
      eepromBuffer.sine_mode_power / 10);
#endif
}
...
void zcfoundroutine() { // only used in polling mode, blocking routine.
  ...
  //			if(thiszctime < (commutation_interval -
  //(commutation_interval>>2))){ 					waitTime = waitTime + commutation_interval -
  //thiszctime; 			    thiszctime = commutation_interval - (commutation_interval>>2);
  //		}else if(thiszctime > (commutation_interval +
  //(commutation_interval>>2))){ 			    waitTime = waitTime - thiszctime -
  //commutation_interval; 			    thiszctime = commutation_interval -
  //(commutation_interval>>2);
  //		}
  ...
  zero_crosses++;
#ifdef NO_POLLING_START // changes to interrupt mode after 2 zero crosses, does
                        // not re-enter
  if (zero_crosses > 2) {
    old_routine = 0;
...
int main(void) {
  ...
  //   startup_max_duty_cycle = startup_max_duty_cycle * TIMER1_MAX_ARR / 2000 +
  //   dead_time_override; // adjust for pwm frequency throttle_max_at_low_rpm =
  //   throttle_max_at_low_rpm * TIMER1_MAX_ARR / 2000; // adjust to new pwm
  //   frequency throttle_max_at_high_rpm = TIMER1_MAX_ARR; // adjust to new pwm
  //   frequency
  if (!eepromBuffer.comp_pwm) {
  ...
  eepromBuffer.use_sine_start = 0;
  target_e_com_time =
      60000000 / FIXED_SPEED_MODE_RPM / (eepromBuffer.motor_poles / 2);
  input = 48;
#endif
  ...
  min_startup_duty =
      min_startup_duty + 200 + ((eepromBuffer.pwm_frequency * 100) / 24);
  minimum_duty_cycle =
      minimum_duty_cycle + 50 + ((eepromBuffer.pwm_frequency * 50) / 24);
  ...
  while (1) {
    ...
    if (eepromBuffer.variable_pwm ==
        1) { // uses range defined by pwm frequency setting
    ...
    if (signaltimeout >
        (LOOP_FREQUENCY_HZ >> 1)) { // half second timeout when armed;
      if (armed) {
        allOff();
        armed = 0;
        input = 0;
    ...
    if (crawler_mode) {
      ...
      if (adjusted_input <
          150) { // startup duty cycle should be low enough to not burn motor
        bemf_timeout = 100;
      } else {
        bemf_timeout = 10;
      }
    ...
    if (desync_check && zero_crosses > 10) {
      if ((getAbsDif(last_average_interval, average_interval) >
        ...
        zero_crosses = 0;
        desync_happened++;
        if ((!eepromBuffer.bi_direction && (input > 47)) ||
    ...
    if (adc_counter > 200) { // for adc and telemetry
      ...
      converted_degrees =
          __LL_ADC_CALC_TEMPERATURE(3300, ADC_raw_temp, LL_ADC_RESOLUTION_12B);
#endif
#ifdef MCU_GDE23
      ADC_DMA_Callback();
      // converted_degrees = (1.43 - ADC_raw_temp * 3.3 / 4096) * 1000 / 4.3 +
      // 25;
      converted_degrees =
          ((int32_t)(357.5581395348837f * (1 << 16)) -
           ADC_raw_temp * (int32_t)(0.18736373546511628f * (1 << 16))) >>
          16;
    ...
    if (stepper_sine == 0) {
      ...
      k_erpm =
          e_rpm /
          10; // ecom time is time for one electrical revolution in microseconds
      ...
      if (low_rpm_throttle_limit) { // some hardware doesn't need this, its on
        ...
        duty_cycle_maximum =
            map(k_erpm, low_rpm_level, high_rpm_level, throttle_max_at_low_rpm,
      ...
      if (input > 48 && armed) {
        ...
        if (input > 48 && input < 137) { // sine wave stepper
          ...
          advanceincrement();
          step_delay = map(input, 48, 120, 7000 / eepromBuffer.motor_poles,
                           810 / eepromBuffer.motor_poles);
          delayMicros(step_delay);
          e_rpm =
              600 / step_delay; // in hundreds so 33 e_rpm is 3300 actual erpm
...
#ifndef PWM_ENABLE_BRIDGE
          duty_cycle =
              (TIMER1_MAX_ARR - 19) + eepromBuffer.drag_brake_strength * 2;
          adjusted_duty_cycle =
              TIMER1_MAX_ARR - ((duty_cycle * tim1_arr) / TIMER1_MAX_ARR) + 1;
          proportionalBrake();
          SET_DUTY_CYCLE_ALL(adjusted_duty_cycle);
          prop_brake_active = 1;
#else
